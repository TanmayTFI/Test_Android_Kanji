/**
 * Kanji rendering and I/O engine - Android OpenGL 1.x GPU buffer, definitions
 *
 * (c) 2010
 */

#ifndef  _KANJI_KBATCHGLES_H
#define  _KANJI_KBATCHGLES_H
...
/**
 * Class modeling OpenGL 1.x GPU buffers
 */

class KBatchGLES : public KBatch {
...
private:
   /**
    * Initialize handling of GPU buffers for OpenGL. Called by KWindowGLES
    *
    * \param lpKWindow game window
    */
   static void setup (KWindowGLES *lpKWindow);

   /** Free all resources used by KBatchGLES instances */
   ...
   /** true if a hardware check has been done yet, to see if we should use VBOs for static vertex buffers or not */
   static bool g_bHardwareDetected;
   
   /** true if we should use VBOs, false if not */
   static bool g_bUseVBOs;
   
   /** Game window */
   static KWindowGLES *g_lpKWindow;
   
   /** List of all KBatchGLES instances */
   static KList<KBatchGLES> g_lBatches;
};

#endif   /* _KANJI_KBATCHGLES_H */
