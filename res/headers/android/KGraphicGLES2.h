/**
 * Kanji rendering and I/O engine - Android OpenGL ES 2.0 GPU graphic definitions
 *
 * (c) 2010
 */

#ifndef  _KANJI_KGRAPHICGLES2_H
#define  _KANJI_KGRAPHICGLES2_H
...
class KGraphicGLES2 : public KGraphic {
...
private:
   /**
    * Initialize handling of GPU textures for OpenGL. Called by KWindowGLES2
    *
    * \param lpKWindow game window
    */
   static void setup (KWindowGLES2 *lpKWindow);

   /** Free all resources used by KGraphicGLES2 instances */
   ...
   /**
    * Bind texture for drawing and enable alpha blending, to be batched
    *
    * \param nTextureIdx index of texture to be bound in this graphic, 0.._nTextures
    */
   void bindTextureWithBlendingForBatch (long nTextureIdx);
   ...
   /** Enable loading textures into this graphic without padding their width and height to a power of two, if supported by the GPU */
   bool _bEnableNonPow2Dimensions;
   ...
   /** Current source alpha blending mode for the color channels of this graphic */
   ...
   KGraphicGLES2 *_lpRequestedAtlas;
   ...
   /** Array of color values for blits */
   static float g_fColorArray[16];
   
   /** Currently bound GPU vertex buffer, NULL for none (the static vertex and UV arrays for non-batched blits are set) */
   static KBatchGLES2 *g_lpCurBoundBatch;

   /** Stack of render targets, topmost is active */
   static KGraphicGLES2 *g_lpCurRenderTarget[4];

   /** List of all KGraphicGLES2 instances */
   static KList<KGraphicGLES2> g_lGraphics;
   
   /** Mutex protecting access to the list of KGraphicGLES2 instances */
   static KSysLock g_graphicsListLock;
};

#endif   /* _KANJI_KGRAPHICGLES2_H */
