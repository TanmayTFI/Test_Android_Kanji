...
*/
  ...
  if (myLang == LN_BRAZILIAN_PORTUGUESE)
  {
...
#else
    subText result = { _text, (float)std::atof(_begin.c_str()), (float)std::atof(_end.c_str()) };
#endif
	return result;
}
...
void freeAssetsStringTable () {
 long i;
   ...
   _nStrings = 0;
	...
	{
		std::string Filename = "BNR\\BNRASSETSTEXTSIZE_" + StringTable::GetMyLangauge() + ".GTR";
        //Filename to uppercase!!
        int strLen = (int) Filename.size(); 
	...
	int GetLineSpacing(std::string key)
	{
		...
		for(SizeType i = 0; i < numObjects; i++)
		{
			...
			{
				width = document["Data"][i]["lineSpacing"].GetInt();
				break;
			}
		}
