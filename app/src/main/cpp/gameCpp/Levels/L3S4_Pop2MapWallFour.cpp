...
const CRectangle  ActionRectGroundBrick(601.0f,256.0f,260.0f,161.0f);
const CRectangle  ActionRectStoneLeaves(674.0f,231.0f,178.0f,206.0f);
...
L3S4_Pop2MapWallFour::L3S4_Pop2MapWallFour(int popupId, HudControl *hud, CHOScene * scene,  int zOrder, CPoint zoomOrigin, std::function<void (Popup*)> popupCloseCallback) : Popup( popupId, hud, scene, zOrder,  zoomOrigin, popupCloseCallback)
{
	//Popup load
	Load("LV3\\S4\\POP2D\\P2D.SCN");
	...
	if(ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DLeavesRemoved)
	{
	...
	if(ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DStoneRemoved)
	{
...
void L3S4_Pop2MapWallFour::OnPopupOpened()
{
	SetupAndShowUI();
}

void L3S4_Pop2MapWallFour::OnPopupCloseBtnClicked()
{
	HideUI();
}

//UI sliding handling
void L3S4_Pop2MapWallFour::SetupAndShowUI()
{
...
void L3S4_Pop2MapWallFour::HideUI()
{
	UINode0->SetEnable(false);
	UINode1->SetEnable(false);
	UINode2->SetEnable(false);
...
void L3S4_Pop2MapWallFour::StartSceneSlideIn(bool fromLeft)
{
	if(ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DLeavesRemoved)
	{
		GetObjectByID("leaf1")->SetEnable(false);
...
void L3S4_Pop2MapWallFour::StartSceneSlideOut(bool toLeft)
{
	slideTimer = 0;
...
void L3S4_Pop2MapWallFour::resetScene()
{
	RestoreXY();
	_popupAlpha = 1.0f;
}
...
void L3S4_Pop2MapWallFour::setLoadNextCallback(std::function<void (Popup*, bool)> _loadNextPopupCB)
{
	LoadNextPopup = _loadNextPopupCB;
}
//UI sliding handling

void L3S4_Pop2MapWallFour::Required()
{

}

void L3S4_Pop2MapWallFour::Update(float ds)
{
	Popup::Update(ds);
	...
	{		
		...
		if(Control::Input->LBclicked())
		{
			...
			if(LoadNextPopup!=nullptr)
				LoadNextPopup(this, true); //load Map Wall 3
		}
	}
	...
	if(!ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DStoneRemoved)
	{
		if(ActionRectGroundBrick.Intersection(&pos))
		{
			...
			if(Control::Input->LBclicked())
			{
				...
				GetObjectByID("stone")->TweenPosTo(-80, -40, -1, 1.25f, 0, true);
				ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DStoneRemoved = true;
			}
		}
	}
	else if(!ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DLeavesRemoved)
	{
		if(ActionRectGroundBrick.Intersection(&pos))
		{
			Cursor::SetMode(CA_HandTake);
			if(Control::Input->LBclicked())
			{
				...
				GetObjectByID("leaf3")->TweenPosTo(-60, 40, -1, 1.0f, 0, true);

				ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DLeavesRemoved = true;
			}
		}
	}
	else if(!ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DSnakeButtonPressed)
	{
		if(ActionRectGroundBrick.Intersection(&pos))
		{
			...
			if(Control::Input->LBclicked())
			{
				...
				ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DSnakeButtonPressed = true;
...
void L3S4_Pop2MapWallFour::HandleInventoryDrop(CPoint dropPos, int itemVal)
{
	GFInstance->WrongInvDropMsg(Hud);
}

void L3S4_Pop2MapWallFour::ProcessHint()
{
	if(GetPopupState()!= ePopupState_Open )
		return;
	...
	else if(!ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DStoneRemoved)
	{
		Hud->InitHintTrailEmit(&ActionRectStoneLeaves);
	}
	else if(!ProfileSystem::Instance->ActiveProfile.L3_S4_IsPop2DSnakeButtonPressed)
...
bool L3S4_Pop2MapWallFour::isTaskLeft()
{
	bool isAnyTaskLeft = false;
	return isAnyTaskLeft;
}

L3S4_Pop2MapWallFour::~L3S4_Pop2MapWallFour()
{
}